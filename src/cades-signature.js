...
class CadesTimestamp extends CadesSignature {
	constructor(model) {
		...
		this._messageImprint = new DigestAlgorithmAndValue(
			model["messageImprint"]
		);
	}
...
class CadesSignerInfo {
	constructor(model) {
		...
		if (model["messageDigest"]) {
			this._messageDigest = new DigestAlgorithmAndValue(
				model["messageDigest"]
			);
		}

		if (model["signature"]) {
			this._signature = new SignatureAlgorithmAndValue(
				model["signature"]
			);
		}
		...
		if (model["signaturePolicy"]) {
			this._signaturePolicy = new SignaturePolicyIdentifier(
