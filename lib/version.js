...
class Version {
   ...
   compareTo(comparedVersion) {
      ...
      while (instanceInternalVersions.length !== comparedInternalVersions.length) {
         if (instanceInternalVersions.length > comparedInternalVersions.length) {
      ...
      for (let i = 0; i < instanceInternalVersions.length; i++) {
         if (instanceInternalVersions[i] > comparedInternalVersions[i]) {
            return 1;
         } else if (instanceInternalVersions[i] < comparedInternalVersions[i]) {
            return -1;
         }
      }
