			...
			exec(_shellEscape(cmdArgs), (err, stdout) => {
				if (err) {
					if (err.code === 1 && this._versionManager.minVersion > new Version('1.0')) {
						...
						reject(new ValidationError(err.code, stdout.split(os.EOL), err));
					} else if (err.code === 127) {
						reject(new InstallationNotFoundError(err.code, code, 'Could not find PKI Express\'s installation.', err));
					}

					reject(new CommandError(err.code, `${stdout.split(os.EOL)}`, err));
					return;
				}
