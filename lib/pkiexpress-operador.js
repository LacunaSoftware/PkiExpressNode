...
class PkiExpressOperator {
	...
	_invoke(command, args, plainOutput) {
		...
		return new Promise((resolve, reject) => {
			exec(_shellEscape(cmdArgs), (err, stdout, stderr) => {
				if (err) {
		...
		if (home) {
			...
			if (system === 'linux') {
				...
				if (!fs.existsSync(path.join(home, 'pkie.dll'))) {
					throw new Error(`The file pkie.dll could not be found on directory ${home}`);
				}

			} else if (!fs.existsSync(path.join(home, 'pki.exe'))) {
				throw new Error(`The file pki.exe could not be found on directory ${home}`);
			}
			...
			if (!home) {
				throw new Error('Could not determine the installation folder ' +
					'of PKI Express. If you installed PKI Express on a custom ' +
					' folder, make sure your chosen folder are specified it ' +
					'on the PkiExpressConfig object');
			}
		}
	...
	addFileReferenceSync(alias, referencePath) {
		...
		if (!referencePath || !fs.existsSync(referencePath)) {
			throw new Error('The provided file path is not valid');
		}
	...
	addTrustedRoot(rootPath) {
		return new Promise((resolve, reject) => {
			...
			if (!rootPath) {
				reject(new Error('The provided trusted root path is not valid'));
			}

			fs.access(rootPath, fs.constants.F_OK, (err) => {
				if (err) {
					reject(new Error('The provided trusted root was not found'));
				}
