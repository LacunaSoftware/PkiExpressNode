...
class Pkcs12GenerationResult {
	...
	constructor(model) {
		this._pfxContentRaw = null;

		if ('pfx' in model) {
			this._pfxContentRaw = new Buffer(model['pfx'], 'base64');
		}
	}

	//region "pfxContentRaw" Accessors

	getPfxContentRaw() {
		return this._pfxContentRaw;
	}
	...
	setPfxContentRaw(value) {
		this._pfxContentRaw = value;
	}

	set pfxContentRaw(value) {
		this._pfxContentRaw = value;
	}

	//endregion

	//region "pfxContentBase64" Accessors

	getPfxContentBase64() {
		if (!this._pfxContentRaw) {
			return null;
		}
		return new Buffer(this._pfxContentRaw).toString('base64');
	}

	get pfxContentBase64() {
		if (!this._pfxContentRaw) {
			return null;
		}
		return new Buffer(this._pfxContentRaw).toString('base64');
	...
	setPfxContentBase64(value) {
		this._pfxContentBase64 = new Buffer(value, 'base64');
	}

	set pfxContentBase64(value) {
		this._pfxContentBase64 = new Buffer(value, 'base64');
	}
	...
	writeToFile(path) {
		return new Promise(resolve => {
			fs.writeFile(path, this._pfx, (err) => {
				if (err) {
					throw new Error(`The provided PDF file could not been stored: ${err}`);
				}
				resolve();
			});
		});
	...
	writeToFileSync(path) {
		fs.writeFileSync(path, this._pfx);
	}
}
