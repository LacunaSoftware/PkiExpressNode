...
class PdfMarker extends PkiExpressOperator {
	...
	apply() {
		...
		return new Promise((resolve, reject) => {
			...
			fs.writeFileSync('C:\\Users\\ismaelm\\Downloads\\temp.json', JSON.stringify(request));
			fs.writeFile(tempFilePath, JSON.stringify(request), (err) => {
				if (err) {
					reject(new Error('Could not create apply marks\'s request file.'));
					return;
				}
