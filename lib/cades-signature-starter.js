...
class CadesSignatureStarter extends SignatureStarter{
   ...
   constructor(config){
       config = config || PkiExpressConfig();
       super(config);
    ...
    start(){
        ...
        transferFile = this.getTransferFileName();

        args = [
            this._fileToSignPath,
            this._certificatePath,
            path.join(this.config._transferDataFolder, transferFile)
        ]
