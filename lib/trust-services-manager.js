...
class TrustServicesManager extends PkiExpressOperator {
	 ...
	 *
	 *The time (in seconds) of the signature session
	 * @param {int} sessionLifetime
	 *
	 * The state that is preserve during the session
	...
	startAuth(
		...
		if (sessionLifetime) {
			...
			args.push(sessionLifetime.toString());
		}
		if (customState) {
	 ...
	 *
	 *  @param {string} state Gets the custom state passed when the trust service authorization session was started
	...
	getCustomState(state) {
		...
		// Invoke command.
		return new Promise((resolve, reject) => {
			this._invoke(Command.GET_SERVICE_AUTH_CUSTOM_STATE, args)
				.then(response => {
					let output = TrustServicesManager._parseOutput(response[0]);
