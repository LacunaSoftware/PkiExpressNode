...
class PKCertificate {
	...
	constructor(model) {
		...
		if (model['pkiArgentina']) {
			this._pkiArgentina = new PkiArgentinaCertificateFields(model['pkiArgentina']);
		}
		if (model['pkiEcuador']) {
			this._pkiEcuador = new PkiEcuadorCertificateFields(model['pkiEcuador']);
		}
	...
	constructor(model) {
		...
		if (qs && qs.length) {
			for (let qualifierModel of qs) {
				this._qualifiers.push(new CertificatePolicyQualifier(qualifierModel));
			}
		}
	}
