...
} = require("./standard-signature-policies");
...
class BaseSigner extends PkiExpressOperator {
	...
	_verifyAndAddCommonOption(args) {
		if (
			_requireTimestamp(this._signaturePolicy) &&
			!this._timestampAuthority
		) {
			throw new Error(
				"The provided policy requires a timestamp authority " +
		...
		if (this._signaturePolicy) {
			...
			// policies.
			if (
				this._signaturePolicy !==
					StandardSignaturePolicies.XML_DSIG_BASIC &&
				this._signaturePolicy !==
			...
			if (
				this._signaturePolicy ===
					StandardSignaturePolicies.COD_WITH_SHA1 ||
				this._signaturePolicy ===
			...
			if (
				...
				this._signaturePolicy ===
					StandardSignaturePolicies.PKI_BRAZIL_PADES_ADR_BASICA_WITH_LTV ||
				this._signaturePolicy ===
