...
class SignatureFinisher extends PkiExpressOperator {
		...
		return new Promise((resolve, reject) => {
			fs.access(filePath, fs.constants.F_OK, (err) => {
				if (err) {
					reject(
						new Error(
							"The provided file to be signed was not found"
						)
					);
					return;
				}
	...
	setFileToSignFromBase64(contentBase64) {
		return new Promise((resolve, reject) => {
			...
			try {
				...
				reject(
					new Error(
						"The provided file to be signed was not Base64-encoded"
					)
				);
				return;
			}
	...
	setTransferFileFromPath(file) {
		return new Promise((resolve, reject) => {
			...
			fs.access(transferPath, fs.constants.F_OK, (err) => {
				if (err) {
					reject(
						new Error("The provided transfer file was not found")
					);
					return;
				}
	...
	setTransferFileFromBase64(contentBase64) {
		return new Promise((resolve, reject) => {
			...
			try {
				...
				reject(
					new Error(
						"The provided transfer file was not Base64-encoded"
					)
				);
				return;
			}
	...
	setDataFileFromBase64(contentBase64) {
		return new Promise((resolve, reject) => {
			...
			try {
				...
				reject(
					new Error("The provided data file was not Base64-encoded")
				);
				return;
			}
	...
	complete(getCert = false) {
		...
		if (getCert) {
			...
			// Invoke command.
			return new Promise((resolve, reject) => {
				this._invoke(Command.COMPLETE_SIG, args)
					.then((response) => {
						// Parse output and return model.
						let output = SignatureFinisher._parseOutput(
							response[0]
						);
