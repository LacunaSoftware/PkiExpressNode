	...
	_invoke(command, args, plainOutput) {
		...
		return new Promise((resolve, reject) => {
			...
			exec(_shellEscape(cmdArgs), options, (err, stdout) => {
				if (err) {
					let code = err.code;
					if (code === ErrorCodes.BAD_SYNTAX && this._versionManager.minVersion > new Version('1.0')) {
						reject(new CommandError(code, `${stdout.split(os.EOL)} ${os.EOL}  ` +
						...
						reject(new InstallationNotFoundError('Could not find PKI Express\'s installation.', err));
					}

					reject(new CommandError(code, `${stdout.split(os.EOL)}`, err));
					return;
				}
	...
	_getPkiExpressInvocationSync() {
		...
		if (home) {
			...
			if (system === 'linux') {
				...
				throw new InstallationNotFoundError(`The file pkie.exe could not be found on directory ${home}`);
			}
