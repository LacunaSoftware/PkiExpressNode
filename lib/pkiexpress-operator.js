...
class PkiExpressOperator {
	constructor(config) {
		...
		this._trustServiceSession = null;
		this._timezone = null;
		this._culture = null;
	}
	...
	_invoke(command, args, plainOutput) {
		...
		if (this._trustServiceSession) {
			cmdArgs.push("--trust-service-session");
			cmdArgs.push(this._trustServiceSession);
			this._versionManager.requireVersion("1.18");
		}

		// Add timezone if set
		if (this._timezone) {
			cmdArgs.push("--timezone");
			cmdArgs.push(this._timezone);
		}

		// Add culture if set
		if (this._culture) {
			cmdArgs.push("--culture");
			cmdArgs.push(this._culture);
		}
		...
		return new Promise((resolve, reject) => {
			let options = {};
			if (this._maxBuffer) {
				options["maxBuffer"] = this._maxBuffer;
			}

			exec(_shellEscape(cmdArgs), options, (err, stdout) => {
				console.log(cmdArgs);
				if (err) {
					let code = err.code;
					if (
						code === ErrorCodes.BAD_SYNTAX &&
						this._versionManager.minVersion > new Version("1.0")
					) {
						reject(
							new CommandError(
								code,
								`${stdout.split(os.EOL)} ${os.EOL}  ` +
						...
						return;
					} else if (code === ErrorCodes.VALIDATION_FAILED) {
						reject(new ValidationError(stdout.split(os.EOL), err));
					} else if (code === ErrorCodes.COMMAND_NOT_FOUND) {
						reject(
							new InstallationNotFoundError(
								"Could not find PKI Express's installation.",
								err
							)
						);
					}
					...
					reject(
						new CommandError(code, `${stdout.split(os.EOL)}`, err)
					);
					return;
				}
			...
			if (system === "linux") {
				if (!fs.existsSync(path.join(home, "pkie.dll"))) {
					throw new InstallationNotFoundError(
						`The file pkie.dll could not be found on directory ${home}`
					);
				}
			} else if (!fs.existsSync(path.join(home, "pkie.exe"))) {
				throw new InstallationNotFoundError(
					`The file pkie.exe could not be found on directory ${home}`
				);
			}
		} else if (system === "win") {
			if (
		...
		return new Promise((resolve, reject) => {
			tmp.file(
				...
				(err, tempFilePath) => {
					if (err) {
						reject(
							new Error(`Could not create temporary file: ${err}`)
						);
						return;
					}
		...
		return new Promise((resolve, reject) => {
			crypto.randomBytes(sizeBuffer, (err, rndBuffer) => {
				if (err) {
					reject(
						new Error(
							`Could not generate transfer file's name: ${err}`
						)
					);
					return;
				}
	...
	addTrustedRoot(rootPath) {
		return new Promise((resolve, reject) => {
			...
			fs.access(rootPath, fs.constants.F_OK, (err) => {
				if (err) {
					throw new Error(
						`The provided trusted root path doesn't exist: ${rootPath}`
					);
				}
	 ...
	 */
	get trustServiceSession() {
		return this._trustServiceSession;
	}
	 ...
	 */
	set trustServiceSession(value) {
		this._trustServiceSession = value;
	}
	 ...
	 */
	set timezone(value) {
		// If value is null or undefined, allow it (clearing the timezone)
		if (value === null || value === undefined) {
			this._timezone = value;
			return;
		}
		
		// Check if the provided value is a valid timezone ID
		const validTimezones = Object.values(TimeZones);
		if (!validTimezones.includes(value)) {
			throw new Error(`Unknown timezone: ${value}. Please use one of the valid timezone IDs from the TimeZones enum.`);
		}
