...
class Signer extends BaseSigner {
		...
		return new Promise((resolve, reject) => {
			fs.access(pkcs12Path, fs.constants.F_OK, (err) => {
				if (err) {
					reject(
						new Error(
							"The provided PKCS #12 certificate file was not found"
						)
					);
					return;
				}
	...
	setPkcs12FromBase64(contentBase64) {
		return new Promise((resolve, reject) => {
			...
			try {
				...
				reject(
					new Error("The provided certificate was not Base64-encoded")
				);
				return;
			}
	...
	setCertFileFromPath(certFilePath) {
		return new Promise((resolve, reject) => {
			fs.access(certFilePath, fs.constants.F_OK, (err) => {
				if (err) {
					reject(
						new Error("The provided certificate file was not found")
					);
					return;
				}
	...
	_verifyAndAddCommonOption(args) {
		...
		if (
			...
			PKCS #12 file, the trust service session, nor the key name was provided"
			);
		}
		...
		if (this._trustServiceSession) {
			args.push("--trust-service-session");
			args.push(this._trustServiceSession);
